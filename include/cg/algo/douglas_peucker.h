...
namespace cg
{
   ...
   double dist(point_2t<Scalar> a, point_2t<Scalar> b, point_2t<Scalar> c) {
      ...
      double rr1 = scal_mul(r1, r1);
      double rr2 = scal_mul(r2, r2);
      double rr12 = scal_mul(r12, r12);
      if (rr1 >= rr2+rr12)
         return sqrt(rr2);
      if (rr2 >= rr1+rr12)
         return sqrt(rr1);
      ...
      if (v1 == 0)
         return 0;
      return abs(vect_mul(r12, r1) / sqrt(rr12));
   }
   ...
   OutIter douglas_peucker(BidIter begin, BidIter end, OutIter out, double eps, bool needOutFirst = true) {
      ...
      douglas_peucker(maxIter, end, out, eps, false);
      return out;
   }
}