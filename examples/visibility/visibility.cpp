...
#include <cg/algo/visibility_graph_n3.h>
...
struct contour_contains_point_viewer : cg::visualization::viewer_adapter
{
   ...
   void draw(cg::visualization::drawer_type & drawer) const override
   {
      ...
      for (size_t i = 0; i < short_path.size(); i++) {
          drawer.draw_line(short_path[i][0], short_path[i][1]);
      }
   ...
   bool on_key(int key)
   {
      ...
      case Qt::Key_Right :
         if(modification_mode_)
         {
            if(++current_polygon_ == (int) contours.size())
               current_polygon_ -= contours.size();
         }
         break;
      default:
...
private:
   ...
   std::vector< cg::segment_2 > short_path;
